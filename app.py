...
with col1:
    ...
    if ITEM == options[1]:  # 이름 설정
        FIRST_NAME_SET = st.selectbox("First_name", ["True", "False"])
        LAST_NAME_SET = st.selectbox("Last_name", ["True", "False"])
        if FIRST_NAME_SET == LAST_NAME_SET == "False":
            st.warning("Least 1 option must be True")
        else:
            RAWS = st.number_input("Raws", 50, 1000, step=50)
            MAX_DUP = st.slider("최대 중복허용(%)", 0, 100, 100)
            NONE_SET = st.number_input("결측치 생성", 0)
...
with col2:
    if st.button("SAVE"):
        ...
        if ITEM == options[0]:
            ...
            if NONE_SET != 0:
                data = make_None(data,NONE_SET)
                        
                # if NONE_SET == "True":
                    
                    
            st.session_state.result[column_name] = data
...
with col3:
    if st.button("GEN"):
        if st.session_state.result:
            st.write(pd.DataFrame(st.session_state.result))
        else:
            st.warning("먼저 데이터를 저장한 후에 테이블을 생성하세요.")
